...
figure
title('Group-level stability scores -- FIR experiment')
hold on
...
for num_std = 1:length(list_std_noise)
    ...
    file_summary = [path_data 'stability_fir' filesep 'std_' num2str(list_std_noise(num_std)) filesep 'stability_group' filesep 'summary_stab_group.mat'];
    data = load(file_summary);
    ind = find(~isnan(data.sil_max));
    plot(ind,data.sil_max(ind),list_color{num_std})    
end
legend(label_std)
print([path_data 'figures_sm' filesep 'fig_contrast_group_fir.pdf'],'-dpdf');
...
for num_std = 1:length(list_std_noise)
    label_std{num_std} = ['std' num2str(list_std_noise(num_std))];
    file_summary = [path_data 'stability_fir' filesep 'std_' num2str(list_std_noise(num_std)) filesep 'stability_ind' filesep 'summary_stab_avg_ind.mat'];
    data = load(file_summary);
...
legend(label_std)
print([path_data 'figures_sm' filesep 'fig_contrast_avg_ind_fir.pdf'],'-dpdf');
...
for num_std = 1:length(list_std_noise)           
    label_scale = ['sci' num2str(list_scales(num_sc,1)) '_scg' num2str(list_scales(num_sc,2)) '_scf' num2str(list_scales(num_sc,3))];    
    label_std{num_std} = ['std' num2str(list_std_noise(num_std))];
    file_stability = [path_data 'stability_fir_std_noise' filesep 'std_' num2str(list_std_noise(num_std)) filesep 'stability_group' filesep 'stability_group_sci' num2str(list_scales(num_sc,1)) '.mat'];    
    data = load(file_stability);
    ind = find(data.nb_classes == list_scales(num_sc,2));
    opt_thresh.thresh = list_scales(num_sc,3);
    part = niak_threshold_hierarchy(data.hier{ind},opt_thresh);
...
for num_std = 1:length(list_std_noise)           
    ...
    file_fdr = [path_data 'stability_fir_std_noise' filesep 'std_' num2str(list_std_noise(num_std)) filesep 'stability_group' filesep label_scale filesep 'fdr_group_average_' label_scale '.mat'];    
    data = load(file_fdr);
